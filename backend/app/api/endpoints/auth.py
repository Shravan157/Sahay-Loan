...
async def register(data: RegisterInput, background_tasks: BackgroundTasks):
    ...
    For provider_admin: company_id is required
    """
    try:
        ...
        # If provider admin — link to company
        if data.role == ROLE_PROVIDER_ADMIN:
            if not data.company_id:
                raise HTTPException(status_code=400, detail="company_id is required for provider_admin role.")
